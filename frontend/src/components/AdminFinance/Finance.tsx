...
import WeekProfitChart from "./WeeklyProfitChart";
import { DateInput } from "@mantine/dates";
import MonthlyProfitChart from "./MonthlyProfitChart";
import { IconArrowsMaximize, IconCoin, IconShirt, IconX } from "@tabler/icons-react";
...
const Finance = () => {
  ...
  // set day profit date
  const [dProfitDay, setDProfitDay] = useState(new Date());

  // set Week profit day
  const [wProfitDay, setWProfitDay] = useState(new Date());

  // set month profit day
  const [mProfitDay, setMProfitDay] = useState(new Date());

  // use react query and fetch data
  const { data, isLoading, isError } = useQuery(["invoiceData"], () => {
    return axios
      .get("http://localhost:3001/invoices/all", {
  ...
  useEffect(() => {
    if (!isLoading) {
      calculateProfit(dProfitDay);
      calculateLast7DaysProfitAndItemCount(wProfitDay);
      calculateMonthlyProfitAndItemCount(mProfitDay);
    }
  }, [isLoading]);

  // if data is fetching this overalay will be shows to the user
  if (isLoading) {
    return <LoadingOverlay visible={isLoading} overlayBlur={4} />;
  }
  ...
  return (
    <>
      ...
      <div className={classes.cardsContainer}>
        <Group position="center" spacing={"lg"} mb={20}>
          ...
          >
            <center>
              {/* Add an Image inside the center tags */}
              <Image
                src={tshirt} // Replace with the actual image path
                alt="items"
                width={120} // Set the width of the image as per your requirement
                height={100} // Set the height of the image as per your requirement
              />
            </center>
            <Text weight={500} size={30}>
          ...
          >
            ...
            <div className={classes.dateInputContainer}>
              <DateInput
                ...
                onChange={(e) => {
                  setWProfitDay(e!!);
                  calculateLast7DaysProfitAndItemCount(e!!);
                }} // Update the onChange function to the new one
                style={{
                  width: "500px",
                  borderRadius: "25px",
                }}
            ...
            {weekItemCount === 0 && weekProfit === 0 ? (
              ...
              <Center>
              <IconShirt
                    color={weekProfitData.datasets[0].borderColor}
                    size={50}
                  />
              </Center>
                <Center>
                  <Text weight={600} size={20} >Sold Items</Text>
                  
                </Center>
                <Center>
                <Text weight={600} size={24} color={"blue"}>{weekItemCount}</Text>
                </Center>
                <Divider mb={10} mx={30}/>
                <Center>
                  <IconCoin color={weekProfitData.datasets[0].borderColor}
                    size={50}/>
                </Center>
                <Center>
                <Text weight={600} size={20} >Profit</Text>
                </Center>
                <Text weight={600} size={24} color="blue">
          ...
          >
            ...
            <div className={classes.dateInputContainer}>
              <MonthPickerInput
                ...
                onChange={(e) => {
                  setMProfitDay(e!!);
                  calculateMonthlyProfitAndItemCount(e!!);
                }} // Update the onChange function to the new one
                style={{
                  width: "500px",
                  borderRadius: "25px",
            ...
            {monthItemCount === 0 && monthProfit === 0 ? (
              ...
              <Center>
              <IconShirt
                    color={monthlyProfitData.datasets[0].borderColor}
                    size={50}
                  />
              </Center>
                <Center>
                ...
                <Center>
                <Text weight={600} size={24} color={"blue"}>{monthItemCount}</Text>
                </Center>
                <Divider mb={10} mx={30}/>
                <Center>
                  <IconCoin color={monthlyProfitData.datasets[0].borderColor}
                    size={50}/>
                </Center>
                <Center>
                <Text weight={600} size={20} >Profit</Text>
